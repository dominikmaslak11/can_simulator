#!/bin/bash
# update_F7.sh – Faza 7: Filtrowanie wstępne i eksport wzorca
# Uruchom w katalogu can_simulator/

set -e

echo "=== Wdrażanie Fazy 7: Filtrowanie szumu + eksport wzorca ==="

# ---------- 1. Nadpisanie controllers/associative_controller.py ----------
...
CONTROLLER_EOF
echo "Zaktualizowano controllers/associative_controller.py (filtrowanie + eksport)"

# ---------- 2. Aktualizacja gui/tabs/associative_tab.py (przycisk eksportu) ----------
...
# a) Dodaj metodę export_pattern (jeśli brak)
if "def export_pattern" not in content:
    export_method = '''
...
'''
    # Wstawiamy przed _refresh_loop lub przed końcem klasy
    if "def _refresh_loop" in content:
        content = content.replace("    def _refresh_loop", export_method + "\n    def _refresh_loop")
    else:
        # Wstawiamy na końcu klasy (przed zamknięciem)
        content = content.rstrip() + "\n" + export_method + "\n"

# b) Dodaj przycisk eksportu w create_widgets (jeśli brak)
if "self.btn_export = ttk.Button" not in content:
    # Szukamy btn_clear i dodajemy obok niego
    old_clear = 'self.btn_clear.pack(side=tk.LEFT, padx=5)'
...
with open("gui/tabs/associative_tab.py", "w") as f:
    f.write(content)
print("Zaktualizowano associative_tab.py o przycisk eksportu.")
PYEOF
else
...
echo ""
echo "=== Faza 7 wdrożona pomyślnie ==="
echo "Nowe funkcje:"
echo "  - Automatyczne odrzucanie cyklicznych ID (szum)"
echo "  - Przycisk 'Eksportuj wzorzec' – zapisuje najlepszego kandydata do JSON"
