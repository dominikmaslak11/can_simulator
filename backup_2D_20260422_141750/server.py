...
class CANWebSocketServer:
    ...
    async def _handler(self, websocket: WebSocketServerProtocol):
        logger.info(f"Nowe połączenie z {websocket.remote_address}")
        if self.token:
            try:
                msg = await asyncio.wait_for(websocket.recv(), timeout=5.0)
                if msg != self.token:
                    logger.warning(f"Nieprawidłowy token od {websocket.remote_address}: '{msg}'")
                    await websocket.close(1008, "Invalid token")
                    return
                else:
                    logger.info(f"Token zaakceptowany od {websocket.remote_address}")
            except asyncio.TimeoutError:
                logger.warning(f"Timeout oczekiwania na token od {websocket.remote_address}")
